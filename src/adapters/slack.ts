/**
 * pi-bridge — Built-in Slack adapter (bidirectional).
 *
 ...
 *
 * Requires:
 *   - App-level token (xapp-...) for Socket Mode — in settings under pi-bridge.slack.appToken
 *   - Bot token (xoxb-...) for Web API — in settings under pi-bridge.slack.botToken
...
export async function createSlackAdapter(config: AdapterConfig, context: AdapterFactoryContext): Promise<ChannelAdapter> {
	...
	// Tokens live in settings under pi-bridge.slack (not in the adapter config block)
	...
	if (!appToken) throw new Error("Slack adapter requires appToken (xapp-...) in settings under pi-bridge.slack.appToken");
	if (!botToken) throw new Error("Slack adapter requires botToken (xoxb-...) in settings under pi-bridge.slack.botToken");
	...
	return {
		...
		async send(message: ChannelMessage): Promise<void> {
			...
			let threadTs = message.metadata?.threadTs as string | undefined;

			if (!threadTs && channelId.includes(":")) {
			...
			if (full.length <= MAX_LENGTH) {
				await sendSlack(channelId, full, threadTs);
				return;
			}
			...
			while (remaining.length > 0) {
				if (remaining.length <= MAX_LENGTH) {
					await sendSlack(channelId, remaining, threadTs);
					break;
				}
				let splitAt = remaining.lastIndexOf("\n", MAX_LENGTH);
				if (splitAt < MAX_LENGTH / 2) splitAt = MAX_LENGTH;
				await sendSlack(channelId, remaining.slice(0, splitAt), threadTs);
